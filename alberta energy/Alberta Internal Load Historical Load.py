...
for start_date, end_date in result:
    print(f"Start: {start_date}, End: {end_date}")
    resp= aeso_instance.get_alberta_internal_load(start_date=start_date,end_date=end_date)

    pdf = pd.DataFrame(resp)
    sparkDF=spark.createDataFrame(pdf) 

    sparkDF.write.mode("append").format("delta").saveAsTable("gshen_catalog.aeso.alberta_internal_load_historical")
